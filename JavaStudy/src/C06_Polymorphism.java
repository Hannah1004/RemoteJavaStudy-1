...
public class C06_Polymorphism {
	...
	public static void main(String[] args) {
		...
		fruit01 = fruit;
		//부모클래스가 fruit이기 때문에 가능하다 / 자식타입은 부모타입이 될 수 있다 (업 캐스팅)
		fruit01 = pineapple;
		
		pine01 = pineapple;
//		pine01 = fruit; <- 불가능하다 / 부모타입은 없는 기능이 많아서 자식타입이 될 수 없다 (다운 캐스팅)
		
		// # 다운 캐스팅이 가능한 경우 
		// 원래 자식 타입이였던 인스턴스가 업캐스팅 되어 있는 경우, 다시 다운캐스팅 될 수 있다 
		...
		// 업캐스팅 된 상태에서도 오버라이드 된 매서드를 사용한다 
		f.eat();
//		f.smash();  부모타입에도 존재하는 매서드를 사용하면 해당 매서드가 오버라이드 된 경우 오버라이드 되어있는 기능을 사용한다 
		// 업캐스팅 된 상태에서는 본래 클래스가 갖고있던 기능을 잠시 잊어버린다 
		// 파인애플에만 존재하는 기능은 잠시 사용할 수 없다 
