...
public class D09_CollectEmail {
	...
	public static void main(String[] args) {
		Pattern validEmail = Pattern.compile("\\w+@\\w+(\\.\\w+)?(\\.\\w+)?");
		...
		System.out.println("총 이메일 사이즈 :" + email.size());
		
		HashSet<String> emailHash = new HashSet<>();
		emailHash.addAll(email);
		
		System.out.println("Hash이메일 사이즈 :" + emailHash.size());
		
		System.out.println("중복 이메일 개수 : " + (email.size() - emailHash.size()));
		...
		for(int i = 0; i < email.size(); ++i) {
			for (int j = i+1; j < email.size(); ++j) {
				if (email.get(i).equals(email.get(j))) {
					email2.add(email.get(i));
				}
			}
		}
		System.out.println("중복 이메일 : " + email2);
